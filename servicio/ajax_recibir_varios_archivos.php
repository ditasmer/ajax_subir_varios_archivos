...
try {
	...
	if(count($_FILES) == 0){
		throw new Exception('El servidor dice: No se ha seleccionado ningun archivo');
	}
	//recorrer la variable FILES para validar y mover cada uno de los archivos que llega al servidor
	$errores = "El servidor dice: Los siguietes archivos superan los 900Kb:\n\n";
	$hayError = false;
	$mensaje = '';
	...
	foreach ($_FILES as $clave => $archivo) {
		...
		if($archivo['size'] > 900000){
			...
			if(move_uploaded_file($archivo['tmp_name'], "../ficheros_servidor/$archivo[name]")){
				$mensaje .= "El servidor dice: fichero $archivo[name] enviado OK";
			} else{
				//throw new Exception('No se ha podido enviar');
				$mensaje .= "El servidor dice: fichero $archivo[name] no se ha podido enviar\n";
			}
		}
	}
